...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "presence"), (snap) => {
      const next = snap.docs.reduce<Record<string, PresenceRow>>((acc, doc) => {
        const data = doc.data() as PresenceRow & { lastSeen?: { toDate: () => Date } };
      ...
      <header className="bg-slate-50/95 border-b border-slate-200 sticky top-0 z-10 backdrop-blur">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="text-sm text-slate-500 hidden sm:flex items-center gap-3">
            ...
            <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1 text-xs font-semibold">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2 text-xs font-medium text-slate-500 justify-start sm:justify-end w-full sm:w-auto">
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1 text-xs font-semibold sm:hidden">
                  <span className="h-2 w-2 rounded-full bg-emerald-500" />
                  {onlineCount} online
                </span>
                    ...
                    {filteredUsers.map((member) => (
                      <div key={member.uid} className="p-4 space-y-3">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                              {(member.providerData || []).map((p) => (
                                <span
                                  key={p}
                                  className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-indigo-50 text-indigo-700 border border-indigo-100"
                                >
                                  {getProviderIcon(p)}
                                </span>
                              ))}
                          ...
                          {isOnline(member.uid) ? (
                            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 px-2 py-1 text-[11px] font-semibold">
                              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                              Online
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200 px-2 py-1 text-[11px] font-semibold">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-slate-600">
                          <div className="flex items-center gap-2">
                            ...
                            <span>Cadastro: {formatDateTime(member.createdAt)}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Calendar className="w-3 h-3 text-slate-400" />
                            <span>Login: {formatDateTime(member.lastSignIn)}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-2 text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-2 rounded-lg hover:bg-indigo-100 transition disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-2 text-xs font-semibold text-red-600 bg-red-50 px-3 py-2 rounded-lg hover:bg-red-100 transition"
                          >
                            ...
                            <td className="px-6 py-4">
                              <div className="space-y-2">
                                <div className="flex items-center justify-between gap-4">
                                  ...
                                  {isOnline(member.uid) ? (
                                    <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 px-2.5 py-1 text-xs font-semibold">
                                      <span className="h-2 w-2 rounded-full bg-emerald-500" />
                                      Online
                                    </span>
                                  ) : (
                                    <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 text-slate-600 border border-slate-200 px-2.5 py-1 text-xs font-semibold">
